...
serial_string_line_reader::serial_string_line_reader(const std::string& port,
                                                     int baud_rate)
    : m_io_service(), m_port(m_io_service), m_istream(this)
{
    ...
    if (!m_port.is_open())
    {
        throw std::runtime_error(std::string("Could not open serial port: ") + port +
                                 ": " + ec.message());
    }
}
